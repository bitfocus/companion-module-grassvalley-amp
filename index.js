	...
	initFeedbacks() {
		const feedbacks = {
			transport: {
				...
				callback: (feedback) =>
					feedback.options.transport_state === this.transport_bits[this.current_transport_status].id,
			},
		}
		...
		this.setActionDefinitions({
			...
			loadclip: {
				...
				callback: async (event) => {
					...
					this.sendCommand(this._buildCommand('4A14', [[clip_name, false, 4]]))
				},
			},
